...
internal class LeaveChatByOwnerNameCommand : BotCommandAction {
    private readonly IWhoIam _whoIam;

    public LeaveChatByOwnerNameCommand(IWhoIam whoIam) : base("leavechatbyownername", "Команда для выхода из чата ботом") {
        _whoIam = whoIam;
    }
    ...
    public override async Task ExecuteActionAsync(ITelegramBotClient botClient, Message message, CancellationToken cancellationToken) {
        ...
        if (string.IsNullOrEmpty(msg)) {
            await botClient.SendTextMessageAsync(message.Chat.Id, "Вы не отправили мне имя владельца группы, из которой я должен выйти");
            return;
        }

        try {
            var chatId = new ChatId(msg);
